...
|-|**Bloque Z.2**: real_results path fix + popup redesign 700x650 + group colors + tabs BebasNeue uppercase + banner 120px + footer BOTTOM|✅ Completado|
...
|2026-06-14|**Bloque Y: Haiti 0-1 Scotland (7° resultado real)**|`output/players.json`: 7 alturas corregidas (Arcus, Ade, Providence, Hendry, McGinn, McLean, Adams) + 52 market values poblados (Placide 155K, Bellegarde 17.6M, McTominay 38M, etc.). `player_ratings.py`: `_SOFASCORE_NAME_MAP` ampliado con Haiti (3) y Scotland (2); `_SEED_DATA` agregado bloque MD1_Haiti_Scotland (30 players, ratings Sofascore). `output/real_results.json`: 7° partido agregado (Haiti 0-1 Scotland, xG 1.21-1.05, McGinn 28', Boston). `gui.py`: BUG 5 fix — `_compact_match_card()` ahora muestra goleadores reales (⚽). **Impacto**: Grupo C Brazil 7pts-Morocco 5pts-Scotland 4pts (3° clasificado)-Haiti 0pts. Argentina campeon, Francia subcampeon, Espana 3°. Messi 11, Mbappe 9, Kane 8. Outputs regenerados, .exe recompilado. Push a master. Commit `81771e2`.|
|2026-06-14|**Bloque Y.2: Botón LEER SINOPSIS**|`gui.py`: nuevo método `_show_synopsis_popup()` con Toplevel + badge header + narrative text + CERRAR button. `_compact_match_card()`: botón LEER SINOPSIS en footer_frame. `analysis.py`: maneja `result_type == "real"` con narrative synopsis. Commit `446c871`.|
|2026-06-14|**Bloque Z: Stats compact cards + fix UI (grupos, goles, ancho) + boton SINOPSIS**|`gui.py`: Stats tab rewrite — `_compact_standings_card()` con colored headers (12 colores A→L, Teko-Bold 18 blanco) y 3-column grid. Mejores Terceros card con 8 ★ teams. Bug 2: Combobox → TODOS/A-L buttons con active state. Bug 3a: `wraplength=150` en nombres de equipo. Bug 3b: goles reales horizontal → vertical (un Label por goal). Bug 5: botón SINOPSIS `bg="#318CE7"` + hover font 7→9 bold. Bug 4: `anchor="n"` + `<Configure>` width bind en Jugadores y Bonus tabs. Info tab: "4 partidos" → "7 partidos". Docs actualizados. .exe recompilado. Push a master.|
|2026-06-14|**Bloque Z.2: popup 700×650 + real_results path + group colors + tabs uppercase + banner 120px + footer BOTTOM**|`gui.py`: `_get_real_results_path()` busca `<exe_dir>/output/real_results.json` cuando frozen; `footer_frame` side=BOTTOM; colores A–L en botones de grupo; `_show_synopsis_popup()` 700×650 centrado con tags veredicto/num/goal; tab labels uppercase; tab font BebasNeue 14; banner target_h 64→120. Commit `d1d75bd`, push a master.|
